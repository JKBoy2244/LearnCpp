...
string_view dateCheck(int dayDate, int monthDate, int yearDate) {
  ...
  bool februaryRange {(yearDate % 400 == 0) || ((yearDate % 4 == 0) && (yearDate % 100 != 0)) ? ((monthDate == 2) && (yearDate >= minYear && yearDate <= 2050) && (dayDate >= 1 && day <= 29)) : ((monthDate == 2) && (yearDate >= 1950 && yearDate <= maxYear) && 
